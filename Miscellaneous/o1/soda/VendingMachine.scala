...
/** The class `VendingMachine` can be used to simulate simple virtual soda vending
  ...
  *
  *  INPUT: "got change: " + machine.sellBottle().getOrElse("none; failed to buy")
  ...
  * OUTPUT: res2: Option[Int] = Some(100)
  *
  *  INPUT: machine
...
class VendingMachine(var bottlePrice: Int, private var bottleCount: Int) {
  ...
  /** (A customer action:) Sells a bottle of virtual soda. This can only be done,
    ...
    *         wrapped in an `Option`; `None` signals an unsuccessful purchase */
  def sellBottle() = {
    if (this.isSoldOut) {
      None
    } else if (!this.enoughMoneyInserted) { // could also use a logical operator; more on that later
      None
    } else {
