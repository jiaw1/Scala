...
object SkyFiles {
  ...
  def parseStarInfo(starAsString: String): Star = {
    val tokens = starAsString.split(";")
    val coords = new StarCoords(tokens(0).toDouble, tokens(1).toDouble)
    val henryDraperID = tokens(3).toInt
    val brightness = tokens(4).toDouble
    // val harvardRevisedID = tokens(5).toInt
    val possibleName = tokens.lift(6)
    new Star(henryDraperID, coords, brightness, possibleName)
  }
