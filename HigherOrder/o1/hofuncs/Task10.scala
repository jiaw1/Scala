...
object Task10 extends App {
  ...
  def blur(pic: Pic) = {
    def blurPixel(x: Int, y: Int) = {
      val neighborhood = pic.pixelsNear(Pos(x, y), 1.45)
      def componentAvg(getComponent: Color => Double) = neighborhood.map(getComponent).sum / neighborhood.size
      Color(componentAvg( _.red ), componentAvg( _.green ), componentAvg( _.blue ))
    }
    pic.transformXY(blurPixel)
  }
