...
/** The class `TribalBot` represents the “brains” of robots that belong to a “robot tribe”
  ...
  *
  * A tribal robot is capable of attacking enemies and thereby converting them to their
  * own tribe. As a consequence, tribes can be quite aggressive as they compete for survival
  * in a robot world.
  *
  *
  * ==Robot Actions==
  ...
  *    is reached.
  *
  *  - The contents of four memory slots (or “registers”) called ''mem'', ''mem2'',
  *    ''radar'' and ''hackline''. The ''mem'' and ''mem2'' slots can be used for any
  *    purpose. The intended use of ''radar'' is to store the readings produced by the
  *    robot's radar. The ''hackline'' slot is meant for configuring hacked robots. See
  *    the text below and class [[Tribe]] for more details.
  *
  *
  * ==Sensors==
  *
  * A tribal bot is (only) capable of seeing the single square directly in front of it.
  * It can determine what there is in that square, but no further.
  *
  * A tribal bot also has a ''radar'' that it can use either ''short-range'' to determine
  * how many enemies or friends are located within exactly two steps of the acting robot,
  * or ''long-range'' to count the memberships of entire tribes in the robot world. Again,
  * more details can be found in the documentation for class [[Tribe]].
  *
  *
  * ==Initial state==
  *
...
class TribalBot(body: RobotBody, val tribe: Tribe) extends RobotBrain(tribe.newName(), body) {
  ...
  override def mayAdvance(direction: CompassDir) = !this.body.neighboringSquare(direction).isUnpassable
  ...
  /** A tribal robot prioritizes hacking: if there is an enemy present directly in front,
    ...
    * during the robot's turns. This involves these two steps:
    *
  ...
  private def runProgram() = {
    while (!this.nextInstruction.endsTurn) {
      this.nextInstruction = this.nextInstruction.execute(this)
    }
    this.nextInstruction = this.nextInstruction.execute(this) // execute the last instruction that ends the turn
  }

  private def runProgram_version2_lazylists() = {
    def executions = LazyList.iterate(this.nextInstruction)( _.execute(this) )
    val turnEndingInstruction = executions.dropWhile( !_.endsTurn ).head
    this.nextInstruction = turnEndingInstruction.execute(this)
  }
  ...
  def determineTribe(another: RobotBrain) = another match {
  ...
  /** "Talks" to a friendly robot that is currently right in front of the acting robot, if
    ...
    *
    * @param lineNumber  a line number referencing the RoboSpeak program of the two friends' shared tribe
    * @see [[isFriend]] */
  ...
  /** "Shouts" to all friendly robots nearby. The message reaches all the friendly robots
    ...
    * @param lineNumber  a new line number for the recipients (see [[talk]])
  ...
  /** "Brainwashes" an enemy robot that is currently right in front of the acting robot,
    ...
    * @see [[enemyInFront]] */
  def hack() = {
    this.enemyInFront.map( _.body ).foreach(this.initiateIntoTribe)
  }


  /** Initializes a newly hacked robot body, giving it a new brain. */
  private def initiateIntoTribe(victim: RobotBody) = {
    victim.brain = Some(new TribalBot(victim, this.tribe))
    this.talk(this.slotValue(Tribe.HackSlot))
  }
  ...
  def callSubprogram(callingLine: Int): Unit = {
    if (this.callStack.size >= 10000) throw new IllegalStateException("Stack overflow: Can't create more stack frames. Probable causes: infinite recursion or a jump outside a subprogram without 'return'.")
    this.callStack.push(new Frame(callingLine))
  }
  ...
  def returnFromSubprogram() = {
    ...
    if (this.callStack.nonEmpty) this.callStack.pop().returnLine else failWithUnderflowError
  }
  ...
  // a more efficient version using views: def longRadar: Seq[RobotBrain] = this.world.robotList.view.flatMap( _.brain )
  ...
  def directedRadar(dirNumber: Int) = {
    val targetDir = LazyList.iterate(this.facing)( _.clockwise )(dirNumber)
    val neighboringLoc = this.location.neighbor(targetDir)
    def isNextToThatLocation(robot: RobotBrain) = neighboringLoc.distance(robot.location) <= 1
    this.shortRadar.filter(isNextToThatLocation)
  }
