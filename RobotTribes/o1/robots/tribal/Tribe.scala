...
/** A “tribe” of robots is esentially a program which makes any robots who follow that program
  ...
  *
  * Tribal programs are written in a programming language called '''RoboSpeak'''. You can think
  * of RoboSpeak as “machine code for tribal robots”. It features a smallish number of simple
  * instructions (commands) which can be combined to build programs that direct tribal robots'
  * actions.
  *
  ...
  * ==Action Instructions==
  *
  * There are four “action instructions” that cause a tribal bot to spend its turn on something
  * concrete:
  *
  ...
  * }}}
  *
  * This program can be paraphrased as: "If you see a wall in front of you, go to line 4
  * (where you spin clockwise before returning to line 1). Otherwise, move one step forward
  * and return to line 1." Either moving or spinning ends the robot's turn. Next turn, it
  * resumes where it left off. In this example, it so happens that the robot will always start
  * its turns by returning to line 1 (because both the ''move'' and ''spin'' commands are
  * followed by ''goto 1'').
  *
  * Only the two action instructions listed above end a tribal bot's turn: a robot can execute
  * any number of non-action instructions during a turn.
  *
  *
  * ==Basic Logic==
  *
...
class Tribe(val name: String, fileName: String) {
  ...
  private[tribal] object Instruction {
    ...
    class Move extends  Simple( _.advanceCarefully() ) with ToNextLine with EndsTurn
