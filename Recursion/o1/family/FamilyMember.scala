...
class FamilyMember(val name: String, val children: Vector[FamilyMember]) {
  ...
  def mostChildren: FamilyMember = this.everyoneBelow.maxBy( _.numberOfChildren )

  private def mostChildren2: FamilyMember = // alternative implementation that avoids constructing everyoneBelow
    if (this.isChildless) this else (this +: this.children.map( _.mostChildren2 )).maxBy( _.numberOfChildren )
  ...
  def everyoneBelow: Vector[FamilyMember] = this +: this.children.flatMap( _.everyoneBelow )
