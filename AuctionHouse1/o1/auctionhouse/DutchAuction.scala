...
class DutchAuction(val description: String, val startingPrice: Int, val decrement: Int, val minimumPrice: Int) {
  ...
  /** Determines if the auction is open, that is, if the item can still be bought. A Dutch auction
    * is always open if nobody has yet bought the item and the auction has not yet expired.
    * @see [[isExpired]] */
  ...
  /** Returns the winner of the auction. That is, returns the name of the person who bought the
    * item, wrapped in an `Option`. `None` is returned if nobody has bought the item yet. */
  def buyer = this.buyerName


  /** Buys the item for the given customer. The auction becomes closed. This only works,
    * however, if the auction was open to begin with. The method returns `true` if the item
